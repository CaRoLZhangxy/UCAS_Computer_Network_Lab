...
struct tcp_sock *sock[MAX_SOCK];
int sock_num = 0;

struct tcp_sock *look_up_sock(int id)
{
	return sock[id];
}
int look_up_id(struct tcp_sock *tsk)
{
	for(int i = 0;i < sock_num;i++)
	{
		if(sock[i]->index == tsk->index)
			return i;
	}
	return -1 ;
}
int alloc_sock_id(struct tcp_sock *tsk)
{
	sock[sock_num] = tsk;
	return sock_num++;
}
...
{
	struct tcp_sock *tsk  = look_up_sock(sockid);
	//close(sockid);
	tcp_sock_close(tsk);
...
#define MIN(x,y) (x<y?x:y)
		...
		ret = tcp_sock_write(tsk,fcache[sv->fidx].file + sv->total_sent, len);
		if (ret < 0) {
...
int handle_connection(int sockid, struct server_vars *sv)
{
	...
	struct tcp_sock *tsk  = look_up_sock(sockid);
	int rd = tcp_sock_read(tsk, buf,HTTP_HEADER_LEN);
	if (rd <= 0) {
		return -1;
	}
	...
	while (sent < len) {
		...
		int ret = tcp_sock_write(tsk,response+sent,len-sent);
		if (ret < 0) {
...
int accept_connection(int listener)
{
	...
	struct tcp_sock *tsk = look_up_sock(listener);
	struct tcp_sock *sk= tcp_sock_accept(tsk);
	int c = alloc_sock_id(sk);
	//printf("c : %d\n",c);
	if (c >= 0) {
		if (c >= MAX_FLOW_NUM) {
...
int create_listening_socket()
{
	...
	struct tcp_sock *sk = alloc_tcp_sock();
	int listener = alloc_sock_id(sk);
	if (listener < 0) {
	...
	ret = tcp_sock_listen(sk, backlog);
	if (ret < 0) {
